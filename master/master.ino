...
void updatePaddles() {
  // Jugador 1
  if (!digitalRead(P1_UP) && paddle1_y > 0) paddle1_y -= 2;
  if (!digitalRead(P1_DOWN) && paddle1_y < SCREEN_HEIGHT - paddle_height) paddle1_y += 2;

  // Jugador 2
  if (!digitalRead(P2_UP) && paddle2_y > 0) paddle2_y -= 2;
  if (!digitalRead(P2_DOWN) && paddle2_y < SCREEN_HEIGHT - paddle_height) paddle2_y += 2;
}
...
void loop() {
  ...
  updateBall();
  drawGame();
  delay(10); // Control de velocidad del juego
}
